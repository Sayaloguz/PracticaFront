...
const Table: FC<TableProps> = ({ useCase, endPointData = {}, token = "" }) => {
  ...
  const columns = data.length > 0 
    ...
    // TODO: Aprovechar que recorremos los datos para crear las URL de las acciones
  const dataTable = data.map((row) => {
    // Filtrar las claves excluidas directamente del objeto
    const filteredRow = Object.keys(row)
      .filter((key) => !excludedColumns.includes(key))
      .reduce((acc, key) => {
        acc[key] = row[key];
        return acc;
      }, {} as Record<string, any>); // Inicializar objeto vacío con tipado
    return filteredRow;
  });
